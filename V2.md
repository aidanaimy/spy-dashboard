# SPY Small-DTE Trading Dashboard - Version 2.0
...
- **Trend Classification**: 20D/50D moving average analysis → Bullish/Bearish/Mixed
...
- **0DTE Permission**: 🚦 RED/YELLOW/GREEN based on trend + gap + range
  - **RED**: Choppy conditions (small gap, low range) → Avoid trading
  - **YELLOW**: Neutral conditions → Proceed with caution
  - **GREEN**: Volatile conditions (large gap, high range) → Favorable for 0DTE
...
- **Tight VWAP Range**: Price within VWAP ± 0.2%

**Action**: If 2+ criteria met → Set to NONE or reduce confidence to LOW
...
- **Pre-Market**: Block all signals → NONE
- **After Hours**: Block all signals → NONE
- **Regular Hours**: Signals allowed (9:30 AM - 4:00 PM ET)

### 4. **Environment Filters** (Regime + Volatility)
- **0DTE Permission**:
  - RED → Force confidence to LOW (even if signal exists)
  - GREEN → Boost MEDIUM to HIGH
  
- **IV/VIX Context**:
  - Low IV (< 15) + Low VIX (< 15) → Reduce MEDIUM to LOW
  - High IV (> 20) or High VIX (> 20) → Boost MEDIUM to HIGH
...
```
1. Data Fetching (Alpaca → yfinance fallback)
   ├─ Daily bars (80-day lookback, cached 5 min)
   └─ Intraday bars (5-min, last 2 days, cached 30 sec)
      └─ Filtered to regular hours only (9:30 AM - 4:00 PM ET)

2. Regime Calculation
   ├─ Calculate 20D/50D MAs
   ├─ Measure gap vs. yesterday's close
   ├─ Calculate today's range
   └─ Determine 0DTE permission (RED/YELLOW/GREEN)

3. Intraday Analysis
   ├─ Calculate VWAP (resets daily)
   ├─ Calculate EMA 9/21 (carries over from yesterday)
   ├─ Determine micro trend
   ├─ Calculate returns & volatility
   └─ Measure distance from VWAP

4. Volatility Context
   ├─ Fetch ATM IV from options chain
   ├─ Fetch VIX level & calculate rank/percentile
   └─ Generate plain-language summary

5. Signal Generation
   ├─ Base signal (CALL/PUT/NONE + confidence)
   ├─ Apply chop detector → Adjust if choppy
   ├─ Apply time filters → Adjust confidence/block
   ├─ Apply market phase gate → Block off-hours
   └─ Apply environment filters → Final confidence adjustment

6. Output
   ├─ Signal direction (CALL/PUT/NONE)
...
### New Features:
- ✅ VIX context integration (rank, percentile, plain-language summaries)
- ✅ 0DTE permission integrated into confidence logic
- ✅ Chop detector with 4-criteria analysis
- ✅ Time-of-day filters (lunch reduction, power hour boost, late-day cutoff)
- ✅ Market phase labeling (Pre-Market, Open Drive, Midday, Afternoon Drift, Power Hour, After Hours)
- ✅ Discord notifications for signal changes
- ✅ Enhanced chart (volume subplot, session markers, high/low of day)
- ✅ Regular hours filtering (9:30 AM - 4:00 PM ET only)
- ✅ EMA carryover from previous day
- ✅ Trade journal delete functionality

### Improvements:
- ✅ More accurate signal confidence (environment-aware)
- ✅ Better signal quality (chop detection reduces noise)
- ✅ Cleaner data (excludes pre-market/after-hours)
- ✅ Continuous indicators (EMA continuity)
- ✅ Better visualization (enhanced chart with session context)
